...
class Store {
  ...
  async init() {
    const saved = this.homey.settings.get('amazon_companion_state');
    this.state = saved && saved.schemaVersion === 1 ? saved : freshState();
    await this.save();
  }

  async save() {
    await this.homey.settings.set('amazon_companion_state', this.state);
  }
  ...
  async createList({ name, marketplace } = {}) {
